...
def mobile_plans():
    try:
        sub_service_id = request.args.get(
            "sub_service_id", default=3, type=int
        )  # Mặc định là gói 4G/5G (id=3)
        sub_services = PlanService.get_sub_services(
            1
        ...
        return render_template(
            ...
            selected_service_id=sub_service_id,
        )
...
def main_plans():
    try:
        plans = PlanService.get_plans_by_service_id(
            2
        )  # Lấy gói cước chính (service_id=2)
        return render_template("Plan_customer/list_main.html", plans=plans)
    except Exception as e:
        return (
            render_template("Plan_customer/list_main.html", plans=[], error=str(e)),
            500,
        )
...
def plan_details(plan_id):
    try:
        ...
        return (
            render_template("Plan_customer/detail.html", plan=None, error=str(e)),
            500,
        )
...
def register_plan():
    ...
    if subscriber.main_balance < price:
        return (
            jsonify(
                {"success": False, "message": "Số dư không đủ để đăng ký gói cước!"}
            ),
            400,
        )
