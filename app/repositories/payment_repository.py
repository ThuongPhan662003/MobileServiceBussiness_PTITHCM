...
class PaymentRepository:
    ...
    def get_all():
        try:
            ...
            data = result[0] if isinstance(result, list) and len(result) == 1 and isinstance(result[0], list) else result
            payments = []
            ...
            for row in data:
                ...
                payment.total_amount = float(total_amount) if total_amount is not None else 0.0
    ...
    @staticmethod
    def search_payments(subscription_id=None, payment_date=None, payment_method=None, is_paid=None):
        try:
            ...
            if payment_method and payment_method != "Tất cả":  # Chỉ lọc nếu không phải "Tất cả"
                if payment_method not in ["Paypal", "Main balance"]:
                    logger.warning(f"Invalid payment_method: {payment_method}, ignoring filter")
                else:
